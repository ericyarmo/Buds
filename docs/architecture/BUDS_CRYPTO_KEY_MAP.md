...
**Last Updated:** January 6, 2026 (Module 0.2: Phone-Based Identity)

---

## 🆕 CRITICAL UPDATE: Phone-Based DID (Module 0.2)

**OLD ARCHITECTURE (Pre-Module 0.2):**
```
DID = did:buds:Base58(Ed25519_pubkey.slice(0,20))
Problem: Each device = unique DID → Multi-device broken
```

**NEW ARCHITECTURE (Module 0.2+):**
```
DID = did:phone:SHA256(phone_number + account_salt)

Where:
- phone_number: E.164 format (e.g., "+16504458988")
- account_salt: 32-byte random hex (generated by relay on first registration)
- Result: Same phone → same DID across ALL devices
```

**Key Properties:**
- ✅ Same DID across all devices with same phone number
- ✅ Salt prevents DID → phone reversal (rainbow table resistant)
- ✅ Relay stores salt in `account_salts` table (keyed by encrypted_phone)
- ✅ Client caches salt in iOS Keychain (fetched once on first launch)

**Multi-Device Identity:**
```
Phone: +16504458988
Account Salt: 7a3f9c8e... (32-byte hex)
  ↓
DID: did:phone:347fd6a98372b922cd0162b558834eeeff4e99c6146f07aabd5311cae53cd43b

Devices using this DID:
├─ iPhone (Device ID: 8F3A2D91...)
│  ├─ Ed25519 keypair (unique to this device)
│  └─ X25519 keypair (unique to this device)
│
└─ iPad (Device ID: 2C7B5F12...)
   ├─ Ed25519 keypair (unique to this device)
   └─ X25519 keypair (unique to this device)
```

**Critical:** Each device has UNIQUE keypairs, but shares the SAME DID (derived from phone).
...
**RELATIONSHIP 1: DID = Phone-Based Identity (Module 0.2)**
```
Phone Number (E.164)
    ↓ [Fetch account_salt from relay]
SHA256(phone + account_salt)
    ↓
DID = "did:phone:347fd6a9..."
```

**✅ FIXED (Module 0.2):** DID is now derived from phone number + salt, NOT from device keypair. This solves multi-device identity!

**RELATIONSHIP 2: One DID → Many Devices (NOW WORKS!)**
```
DID: "did:phone:347fd6a9..."
...
**How it works:**
1. User registers phone number with Firebase Auth
2. First device calls `/api/account/salt` → relay generates random salt
3. Device computes DID = SHA256(phone + salt)
4. Device registers with relay: `{device_id, owner_did: DID, pubkey_ed25519, pubkey_x25519}`
5. Second device: calls `/api/account/salt` → gets SAME salt (keyed by encrypted_phone)
6. Second device computes SAME DID → registers as separate device under same DID

**⚠️ SECURITY NOTE**: DID is pseudonymous but linked to phone number. If relay DB + secrets leak, phones can be exposed. Mitigation: AES-256-GCM encryption (Module 0.3).
